...
private class ActorAttachments private constructor(val actor: Actor) {
    companion object {
...
fun Actor.onActivation(action: () -> Unit): Actor {
...
class OnClickListener(val function: (event: InputEvent?, x: Float, y: Float) -> Unit) : ClickListener() {
    override fun clicked(event: InputEvent?, x: Float, y: Float) {
        function(event, x, y)
    }
}
...
fun Actor.onClickEvent(function: (event: InputEvent?, x: Float, y: Float) -> Unit) {
    this.addListener(OnClickListener(function))
}

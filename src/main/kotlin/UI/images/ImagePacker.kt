...
internal object ImagePacker {
    ...
    fun packImages(isRunFromJAR: Boolean) {
        ...
        if (!isRunFromJAR)
            packImagesPerMod(".", "Images", defaultSettings)
    ...
    private fun packImagesPerMod(input: String, output: String, defaultSettings: TexturePacker.Settings) {
        if (!File("$input${File.separator}Images").exists()) {
            println("No input folder found, skipping texture packing")
            return
        } else { // So we don't run this from within a fat JAR
            println("Starting to pack textures")
        }

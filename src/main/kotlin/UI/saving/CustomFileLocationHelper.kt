...
abstract class CustomFileLocationHelper {
    ...
    fun saveGame(
        ...
        saveCompleteCallback: (QuantumFiles.CustomSaveResult) -> Unit = {}
    ) {
     ...
     */
    fun loadGame(loadCompleteCallback: (QuantumFiles.CustomLoadResult<String>) -> Unit) {
...
private fun callLoadCallback(loadCompleteCallback: (QuantumFiles.CustomLoadResult<String>) -> Unit,
                             location: String? = null,
                             gameData: String? = null,
                             exception: Exception? = null) {
    val result = if (location != null && gameData != null && exception == null) {
        QuantumFiles.CustomLoadResult(location to gameData)
    } else {
        QuantumFiles.CustomLoadResult(null, exception)
    }
    Concurrency.runOnGLThread {
        loadCompleteCallback(result)
    }
}
private fun callSaveCallback(saveCompleteCallback: (QuantumFiles.CustomSaveResult) -> Unit,
                             location: String? = null,
                             exception: Exception? = null) {
    Concurrency.runOnGLThread {
        saveCompleteCallback(QuantumFiles.CustomSaveResult(location, exception))
    }
}
